...
import type { CISession } from "@/lib/competitive-intel/types";
...
export function SavedSessions({ workspaceId, onSelectSession }: Props) {
  const [sessions, setSessions] = useState<CISession[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`/api/competitive-intel/sessions?workspaceId=${workspaceId}`)
      .then((r) => r.json())
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-6 space-y-2">
      {sessions.map((session) => {
        ...
        const competitors = session.config.competitors?.map((c) => c.name).join(", ") ?? "";
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                {competitors && (
                  <>
                    ...
                    <span className="text-xs text-muted-foreground truncate">{competitors}</span>
                  </>
                )}
                <span className="text-muted-foreground/30">·</span>
