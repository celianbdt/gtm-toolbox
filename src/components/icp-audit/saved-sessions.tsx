...
import type { ICASession } from "@/lib/icp-audit/types";
...
export function SavedSessions({ workspaceId, onSelectSession }: Props) {
  const [sessions, setSessions] = useState<ICASession[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`/api/icp-audit/sessions?workspaceId=${workspaceId}`)
      .then((r) => r.json())
      ...
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">
            Run your first ICP audit to see it here.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-6 space-y-2">
      {sessions.map((session) => {
        ...
        const segments = session.config.segments?.map((s) => s.name).join(", ") ?? "";
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                {segments && (
                  <>
                    ...
                    <span className="text-xs text-muted-foreground truncate">{segments}</span>
                  </>
                )}
                <span className="text-muted-foreground/30">·</span>
