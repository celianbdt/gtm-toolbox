...
import type { OBSession, OBSessionConfig, OBSessionOutput, OBOutputType } from "./types";
import { OB_ANALYST_SLUGS } from "./agents";

export async function getOBAnalystTemplates(): Promise<AgentConfig[]> {
  const supabase = createAdminClient();
  const { data, error } = await supabase
    .from("agent_configs")
    .select("*")
    .eq("is_template", true)
    .in("slug", [...OB_ANALYST_SLUGS])
    .order("created_at");
...
export async function createOBSession(payload: {
  workspace_id: string;
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) throw error;
  return data as OBSession;
}

export async function getOBSession(sessionId: string): Promise<OBSession> {
  const supabase = createAdminClient();
  const { data, error } = await supabase
    .from("tool_sessions")
    .select("*")
...
export async function updateSessionPhase(
  ...
  const config = session.config as OBSessionConfig;
  const newConfig = { ...config, current_phase: phase };
  const isComplete = phase === "complete";

  const { error } = await supabase
    .from("tool_sessions")
    .update({ config: newConfig, status: isComplete ? "concluded" : "active" })
    .eq("id", sessionId);
  if (error) throw error;
}
...
export async function insertSessionOutput(output: {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) throw error;
  return data as OBSessionOutput;
}

export async function getSessionOutputs(sessionId: string): Promise<OBSessionOutput[]> {
  const supabase = createAdminClient();
  const { data, error } = await supabase
    .from("session_outputs")
    .select("*")
